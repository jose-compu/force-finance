...
/**
 * @title GMXFuturesManager
 * @dev Optimized GMX Futures Manager for cost-effective LST hedging
 * 
 * Key Advantages over Perpetuals:
 * - No funding rates (saves ~$24k annually)
 * - Lower execution costs
 * - Fixed expiration dates
 * - Better capital efficiency (10% vs 15% collateral)
 * - Simplified position management
 */
contract GMXFuturesManager is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;
    using Math for uint256;

    // GMX Protocol Addresses (Avalanche)
    ...
    constructor(address _oracleManager, address _usdcE) {
        ...
        // GMX addresses on Avalanche
        gmxRouter = 0x5F719c2F1095F7B9fc68a68e35B51194f4b6abe8;
        gmxVault = 0x9ab2De34A33fB459b538c43f251eB825645e8595;
        gmxPositionRouter = 0xffF6D276Bc37c61A23f06410Dce4A400f66420f8;
    ...
    /**
     * @dev Adjust futures position size
     */
    function adjustFuturesPosition(
        bytes32 positionKey,
        uint256 sizeDelta,
        bool isIncrease
    ) external onlyStrategy nonReentrant returns (uint256 newSize, uint256 collateralDelta) {
        require(activeFuturesPositions[positionKey], "Position not found");
        ...
        if (isIncrease) {
            newSize = position.size + sizeDelta;
            collateralDelta = sizeDelta * FUTURES_COLLATERAL_RATIO / 10000;
            
            require(newSize <= maxPositionSize, "Position size exceeds limit");
            require(IERC20(usdcE).balanceOf(strategy) >= collateralDelta, "Insufficient collateral");
            
            IERC20(usdcE).safeTransferFrom(strategy, address(this), collateralDelta);
            position.collateral = position.collateral + collateralDelta;
        } else {
            require(sizeDelta <= position.size, "Cannot decrease more than current size");
            newSize = position.size - sizeDelta;
            collateralDelta = sizeDelta * FUTURES_COLLATERAL_RATIO / 10000;
            
            position.collateral = position.collateral - collateralDelta;
            IERC20(usdcE).safeTransfer(strategy, collateralDelta);
        }
        
        position.size = newSize;
        position.lastRebalanceTime = block.timestamp;
        
        // Execute GMX adjustment
        _executeGMXFuturesAdjust(positionKey, position.token, sizeDelta, position.isLong, isIncrease);
        
        emit FuturesPositionAdjusted(positionKey, newSize, collateralDelta, isIncrease);
    }
    ...
    /**
     * @dev Execute GMX futures position adjustment
     */
    function _executeGMXFuturesAdjust(
        bytes32 /* positionKey */,
        address token,
        uint256 sizeDelta,
        bool isLong,
        ...
        if (isIncrease) {
            ...
            IGMXPositionRouter(gmxPositionRouter).createIncreasePosition{value: executionFee}(
                path,
                token,
                0, // amountIn (collateral already transferred)
                0, // minOut
                sizeDelta,
                isLong,
                0, // acceptablePrice (market order)
